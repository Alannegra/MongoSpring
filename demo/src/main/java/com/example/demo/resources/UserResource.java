...
public class UserResource {
    ...
    @GetMapping("{id}")
    public UserDTO user(@PathVariable("id") BigInteger id){return userController.getUser(id);}
    @PostMapping
    public void addUser(@RequestBody UserDTO userDTO){
        userController.addUser(userDTO);
    }
    @PutMapping("{id}")
    public void putUser(@RequestBody UserDTO userDTO, @PathVariable("id") BigInteger id){userController.putUser(userDTO, id);}
    @DeleteMapping("{id}")
    public void deleteUser(@PathVariable("id") BigInteger id){
        userController.deleteUser(id);
    }
    @GetMapping("{id}/email")
    public Map<String, String> email(@PathVariable("id") BigInteger id){return Collections.singletonMap("email",userController.getUser(id).getEmail());}
